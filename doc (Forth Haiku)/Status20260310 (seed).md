Create Obsidian slides from the following. I will add an appendix with actual generated code
# What Have I Learned from Forth Haiku Thus Far? (20260310)

- 2-stage compilation
- basic shader code does not carry state

- need to create 2 buffers and swap between them to access previous state
- “state” buffer in GPU can use “texture” memory, to implement state buffers
- “game of life” is a cellular automaton that requires “state”, hence, needs more than what Forth Haiku provides
- viewer.html as slave can support some built-in objects and can execute GLSL

- command line can send key-by-key commands at viewer 

- Claude can build viewer.html for me (I’m ignorant of the issues, Claude greatly lessens my learning curve)
...
- websocket daemon started before pipeline

- open websocket and keeps it open
- every time it receives stdin input terminated by ‘// —EOF—‘, it sends the text into the websocket
- file_watcher appends '\n\\ —EOF---\n' to every file send (this is Forth Haiku comment syntax)
- compiler, optimizer, wrapper use ‘// —EOF—‘ (this is Javascript syntax)
...
## First stage - “dumb” code generation

Each Forth “word” is compiled by looking up the “word” in a dictionary, then appending the associated text (list of lines of JS code, using a JS stack) to the generated code buffer

## Second stage

Generated code is examined line by line using REGEX to match for “.push” and “.pop”. 

Pushes are converted to temporary JS variables _and_ memo’ed onto an internal, “simulated” stack by variable name, e.g. “temp1”, “temp2”, etc.

Pops are converted to variable names popped from “simulated” stack.

# Example Code (CPU pipeline)
