var go = function(   time_val, time_delta_val,   xpos, ypos,   mouse_x, mouse_y,   button_val,   memory) {
    ...
    function hasbit(val, b) {     b = Math.floor(b);     return mod(val, Math.pow(2.0, b + 1)) >= Math.pow(2.0, b);   }
    function sample(x, y) {}
    ...
    function load(addr) {     return memory[mod(Math.floor(addr), 16)]; }
    function mod(v1, v2) {     return v1 - v2 * Math.floor(v1 / v2);   }
    var work1, work2, work3, work4;
    var temp1 = 4.0;
    return [temp1, 0.0, 0.0, 1.0];
};
go
